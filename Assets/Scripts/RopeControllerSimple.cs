...
public class RopeControllerSimple : MonoBehaviour {
    ...
    void Start() {
        springJoint = connectedTo.GetComponent<SpringJoint>();
    ...
    private void DisplayRope() {
        ...
        Vector3 A = connectedTo.position;
        Vector3 D = hangingFromRope.position;

        //Upper control point
        //To get a little curve at the top than at the bottom
        Vector3 B = A + connectedTo.up * (-(A - D).magnitude * 0.1f);
        //B = A;

        //Lower control point
        Vector3 C = D + hangingFromRope.up * ((A - D).magnitude * 0.5f);
